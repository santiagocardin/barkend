...
public class NoisyBarksEventsProcessor {
	...
	public Function<KStream<String, BarkingDogEvent>, KStream<String, TooNoisyEvent>> process() {
		...
		return event -> event
				...
				.count()
				.toStream()
				.peek((key, value) -> log.info("Dogs barked {} times during the last {}",
						value, applicationConfig.getTimeWindow()))
				.flatMap((key, value) -> {
					List<KeyValue<String, TooNoisyEvent>> result = new ArrayList<>();
					if (value != null && value > applicationConfig.getMaxBarksAllowed()) {
						log.info("Barking limit of {} times per {} exceeded. Creating new TOO_NOISY event...",
								applicationConfig.getMaxBarksAllowed(), applicationConfig.getTimeWindow());
						result.add(new KeyValue<>("alarm", TooNoisyEvent.builder().reason("F* dogs").build()));
					}
					return result;
				});
		// @formatter:on
