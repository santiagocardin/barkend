...
class BarkDetectorServiceTest {
  ...
  @Autowired BarkDetectorService barkDetectorService;

  @MockBean S3Repository s3Repository;

  @MockBean SeldonClient seldonClient;

  @MockBean BarkingDogEventProducer eventProducer;
  ...
  public void processOneDogBarking() throws IOException {
    ...
    assertEquals(
        AudioCategory.ONE_DOG_BARKING, clip.getMostProbablePrediction().get().getCategory());
  ...
  public void processManyDogsBarking() throws IOException {
    ...
    assertEquals(
        AudioCategory.MANY_DOGS_BARKING, clip.getMostProbablePrediction().get().getCategory());
    Mockito.verify(eventProducer, Mockito.times(1)).sendBarkingDogEvent(any());
    Mockito.verify(s3Repository, Mockito.times(1)).addTagToFile(any(), any());
  }
